...
//   Master synthesizable Verilog-2001 RTL specification.
//
...
//            Auto-zero: controls rise at cnt=63 (one cycle before en_LP falls),
//                       stay high at cnt=0, drop at cnt=1 (mid-gap), low until cnt=63.
...
// Control pulse straddles the LP falling edge: last LP cycle plus first active cycle
...
always @(posedge clk_i or negedge res_n) begin
  if (!res_n) begin
    ...
    // Startup flag clears after first full counter cycle
    if (cnt == 4'd15) begin
      startup <= 1'b0;
    end
  end
...
always @(*) begin
  ...
  if (c_DFT_en_LP) begin
    ...
    // Auto-zero mode:
    // BGR is high at cnt=0, low at cnt=1..2, then alternates with cnt[0] (high on odd cycles)
    ...
    // then alternates with cnt[0] (high on odd cycles)
    if (startup && (cnt < 4'd2)) begin
      oc_ctrl_cp = 1'b1;
    end else begin
      oc_ctrl_cp = cnt[0];
